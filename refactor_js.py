...
text = re.sub(r"const sdgCsv = \$\('sdg-csv-btn'\); if \(sdgCsv\) sdgCsv\.href = '/api/analytics/sdg-alignment/export\.csv' \+ qs;\n\s*", '', text)
text = re.sub(r"else if \(currentAtab === 'sdg'\) \{ sdgLoaded = false; loadSDGTab\(\); \}", '', text)
