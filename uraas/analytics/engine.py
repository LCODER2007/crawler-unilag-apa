...
from uraas.database import (
    SessionLocal, Item, Author, Collection, Community, File,
    db_year, db_year_month
)
from uraas.config.institutions import get_registry
from uraas.utils.ai_classifier import (
    classify_special_collections, extract_keywords,
    extract_trends_from_corpus, SPECIAL_COLLECTIONS,
)
...
AFRICAN_LANG_CODES = {
    ...
    "am": "Amharic", "so": "Somali", "rw": "Kinyarwanda", "sn": "Shona",
    "zu": "Zulu", "xh": "Xhosa", "af": "Afrikaans", "st": "Sesotho",
    "tn": "Setswana", "ts": "Tsonga", "ss": "Swati", "ve": "Venda",
    "nr": "Ndebele", "ff": "Fula", "wo": "Wolof", "bm": "Bambara",
    "ln": "Lingala", "kg": "Kongo", "lua": "Luba", "om": "Oromo",
}
...
STOP_WORDS = {
    "the","and","for","with","this","that","from","have","been","were","their",
    "which","these","about","other","into","than","more","such","some","what",
    "when","where","there","also","using","used","study","show","paper","research",
    "analysis","findings","results","between","effect","impact","based","data",
    "method","approach","model","system","review","case","report","among","within",
    "across","during","after","before","through","while","both","each","only",
    "very","well","high","low","new","large","small","significant","different",
    "similar","total","however","therefore","thus","hence","although","despite",
}
...
class URAASAnalyticsEngine:
    ...
    def get_papers_by_faculty_and_department(self, institution: Optional[str] = None) -> Dict:
        inst_name = self._resolve_institution_name(institution)
        session = SessionLocal()
        try:
                ...
                for coll in comm.collections:
                    ...
                    for p in papers:
                        ...
                        paper_list.append({
                            ...
                            "download_url": f"/api/papers/{p.id}/download" if f else None,
                        })
                    if paper_list:
            ...
            if unclassified:
                tree["Unclassified"] = {"General": [
                    {"id": p.id, "title": p.title or "Untitled", "doi": p.doi or "",
                     "url": p.url or "", "docid": p.docid or "", "has_local_pdf": False}
    ...
    def get_papers_by_year_faculty(self, institution: Optional[str] = None) -> List[Dict]:
        inst_name = self._resolve_institution_name(institution)
        session = SessionLocal()
        try:
            q = session.query(
                db_year(Item.publication_date),
                Community.name,
                func.count(Item.id)
    ...
    def get_open_access_breakdown(self, institution: Optional[str] = None) -> List[Dict]:
        inst_name = self._resolve_institution_name(institution)
        session = SessionLocal()
        try:
            ...
            for it in items:
                if "openAccess" in (it[0] or ""): counts["Open Access"] += 1
                else: counts["Restricted"] += 1
            return [{"label": k, "value": v} for k, v in counts.items()]
        finally:
            session.close()
            
    def get_authors_by_papers(self, limit: int = 10, institution: Optional[str] = None) -> List[Dict]:
        inst_name = self._resolve_institution_name(institution)
        session = SessionLocal()
        try:
    ...
    def get_faculty_oa_breakdown(self, institution: Optional[str] = None) -> List[Dict]:
        ...
        try:
            ...
            for fac, rights in rows:
                if "openAccess" in (rights or ""): facs[fac]["oa"] += 1
                else: facs[fac]["restricted"] += 1
            return [{"faculty": k, "oa": v["oa"], "restricted": v["restricted"]} for k, v in facs.items()]
        finally:
            session.close()
                ...
                for hit in hits:
                    ...
                    try:
                        ...
                        sdg_buckets[num].append({
                            ...
                            'score': hit['score'], 'keywords': hit['matched_keywords']
                        })
    ...
    def get_sdg_csv_data(self) -> List[List]:
        ...
        """
        rows = [['SDG Number', 'SDG Name', 'Paper Count', 'Paper Title', 'Score', 'Matched Keywords']]
        try:
    ...
    def get_keyword_cloud(self, top_n: int = 60, institution: Optional[str] = None) -> List[Dict]:
        ...
        try:
            ...
            keywords = extract_keywords(
                combined_title, combined_abstract,
                top_n=top_n, all_texts=all_texts
            )
            result = [{'word': k['word'], 'count': k['count'], 'score': k['score']} for k in keywords]
            analytics_cache.set(cache_key, result)
            return result
        except Exception as e:
    ...
    def get_institution_leaderboard(self) -> List[Dict]:
        ...
        try:
            ...
            inst_rows = session.query(
                Item.institution, func.count(Item.id).label('total')
            ).filter(Item.institution.isnot(None)).group_by(Item.institution).all()

            leaderboard = []
            for inst_name, total in inst_rows:
                oa = session.query(Item).filter(
                    Item.institution == inst_name,
                    Item.dc_rights.like('%openAccess%')
                ).count()
                authors = session.query(func.count(func.distinct(Author.id))).join(
                    Author.items
                ).filter(Item.institution == inst_name).scalar() or 0
    ...
    def get_linguistic_diversity_index(self) -> Dict:
        ...
        session = SessionLocal()
        try:
            items = session.query(Item.language_code, Item.is_african_language, Item.dc_language).all()
            total = len(items)
            if total == 0:
    ...
    def get_patent_velocity(self) -> Dict:
        ...
        try:
            ...
            avg = round(sum(v["days_to_patent"] for v in velocities) / len(velocities)) if velocities else None

            # Distribution buckets
            buckets = {"< 1 year": 0, "1-2 years": 0, "2-5 years": 0, "> 5 years": 0}
            for v in velocities:
                d = v["days_to_patent"]
                if d < 365:       buckets["< 1 year"] += 1
                elif d < 730:     buckets["1-2 years"] += 1
                elif d < 1825:    buckets["2-5 years"] += 1
            ...
            return {
                ...
                "velocity_distribution": [{"range": k, "count": v} for k, v in buckets.items()],
            }
        except Exception as e:
            logger.error("get_patent_velocity: %s", e)
            return {"total_patents": 0, "avg_days_to_patent": None, "fast_movers": [], "velocity_distribution": []}
        finally:
            session.close()
    ...
    def get_docid_coverage(self) -> Dict:
        ...
        try:
            ...
            by_type = session.query(
                Item.content_type, func.count(Item.id)
            ).filter(Item.docid.isnot(None)).group_by(Item.content_type).all()

            return {
                "total_papers": total,
                "docid_assigned": with_docid,
                "coverage_percent": coverage,
                "by_content_type": [{"type": r[0] or "research_paper", "count": r[1]} for r in by_type],
            }
        except Exception as e:
            logger.error("get_docid_coverage: %s", e)
            return {"total_papers": 0, "docid_assigned": 0, "coverage_percent": 0, "by_content_type": []}
        finally:
            session.close()

    def get_special_collections_metrics(self, institution: Optional[str] = None) -> Dict:
        """
        ...
        try:
            ...
            return {'summary': [], 'total_special_items': 0, 'total_repository_items': 0}
        finally:
            session.close()
    ...
    def get_author_network(self, author_name: Optional[str] = None, limit: int = 30) -> Dict:
        """
        ...
        try:
            if author_name:
                ...
                collaborators = {e["source"] for e in edge_list} | {e["target"] for e in edge_list}
                ...
                return {
                    ...
                    "edges": [{"source": k[0], "target": k[1], "weight": v} for k, v in edges.items()],
                }
        except Exception as e:
