    ...
    def _ensure_institution(self, spider):
        ...
        if not institution_config:
            spider.logger.warning(
                f"Institution '{institution_name}' not found, using default (UNILAG)"
            )
        ...
        spider.logger.info(
            f"Affiliation filter initialized for {institution_config.name}"
        )
    ...
    def process_item(self, item, spider):
        ...
        try:
            ...
            for author in authors:
                try:
                    if self.current_validator.is_staff_member(
                        author, fuzzy_threshold=75
                    ):
            ...
            item["unilag_staff_authors"] = (
                matching_staff  # Legacy field for backward compatibility
            )
            ...
            try:
                ...
                if isinstance(affiliations, list):
                    if any(
                        self.is_institution_affiliated(aff)
                ...
                if not has_affiliation:
                    raw_text = (
                        " ".join(str(a) for a in affiliations if a)
                        if isinstance(affiliations, list)
                        else ""
                    )
