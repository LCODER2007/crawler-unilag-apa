...
from uraas.database import SessionLocal,Item,File,Author,Community,Collection,db_year,db_year_month
...
def papers_tree():
    try:
        ...
        return jsonify({"status":"success","data":analytics.get_papers_by_faculty_and_department(institution=institution)})
    except Exception as e:
...
def get_paper(item_id):
    ...
    try:
        ...
        collections = [{"id":c.id,"name":c.name,"faculty":c.community.name if c.community else "Unknown"} for c in item.collections]
        return jsonify({
            "id":item.id,"docid":item.docid or "","title":item.title or "Untitled",
            "abstract":item.abstract or "","doi":item.doi or "","url":item.url or "",
            ...
            "dc":{"title":item.dc_title or "","date_issued":item.dc_date_issued or "",
                  "identifier_uri":item.dc_identifier_uri or "","identifier_doi":item.dc_identifier_doi or "",
                  "description_provenance":item.dc_description_provenance or "","rights":item.dc_rights or ""},
            "file":{"has_local_pdf":file_record is not None,
                    ...
                    "sha256":file_record.sha256_hash if file_record else None} if file_record else {"has_local_pdf":False},
...
def export_single_bibtex(item_id):
    ...
    try:
        ...
        return Response(bibtex, mimetype='text/plain', headers={'Content-Disposition':f'attachment; filename=paper_{item_id}.bib'})
    except Exception as e:
...
def top_authors_analytics():
    ...
    institution = request.args.get('institution')
    return jsonify(analytics.get_authors_by_papers(limit=limit, institution=institution))
...
def recent_papers():
    ...
    try:
        ...
        return jsonify([{"id":i.id,"title":i.title,"doi":i.doi,"authors":[a.name for a in i.authors[:3]],"created_at":i.created_at.isoformat() if i.created_at else None,"is_oa":'openAccess' in (i.dc_rights or '')} for i in items])
    finally:
        session.close()
...
def papers_by_year_faculty():
    session = SessionLocal()
    try:
        rows = session.query(db_year(Item.publication_date).label('year'),Community.name.label('faculty'),func.count(Item.id).label('count')).join(Item.collections).join(Collection.community).filter(Item.publication_date.isnot(None)).group_by('year',Community.name).order_by('year').all()
...
def faculty_oa_breakdown():
    session = SessionLocal()
    try:
        rows = session.query(Community.name,Item.dc_rights,func.count(Item.id).label('count')).join(Item.collections).join(Collection.community).group_by(Community.name,Item.dc_rights).all()
        result = {}
...
def impact_metrics():
    ...
    try:
        ...
        q_years = session.query(db_year(Item.publication_date)).filter(Item.publication_date.isnot(None))
        if inst_name:
...
def analytics_search():
    ...
    try:
        ...
        if q:
            author_subq = session.query(Item.id).join(Item.authors).filter(Author.name.ilike(f'%{q}%')).subquery()
            q_obj = q_obj.filter(or_(Item.title.ilike(f'%{q}%'),Item.abstract.ilike(f'%{q}%'),Item.doi.ilike(f'%{q}%'),Item.id.in_(author_subq)))
        if faculty:
        ...
        return jsonify([{"id":i.id,"title":i.title,"doi":i.doi,"abstract_snippet":(i.abstract or '')[:200] if q and i.abstract and q.lower() in (i.abstract or '').lower() else None,"authors":[a.name for a in i.authors[:4]],"year":i.publication_date.year if i.publication_date else None,"is_oa":'openAccess' in (i.dc_rights or ''),"faculty":i.collections[0].community.name if i.collections else None} for i in items])
    finally:
        session.close()
...
def list_faculties():
    ...
    try:
        ...
        if inst_name:
            q = q.filter(Community.institution.ilike(f'%{inst_name}%') | Community.name.ilike(f'%{inst_name}%'))
        rows = q.all()
...
def authors_search():
    ...
    try:
        inst_name = analytics._resolve_institution_name(institution) if institution else None
        query = session.query(Author.name, func.count(Item.id)).join(Author.items).filter(Author.name.ilike(f'%{q}%'))
        if inst_name:
            query = query.filter(Item.institution.ilike(f'%{inst_name}%'))
        authors = query.group_by(Author.name).order_by(desc(func.count(Item.id))).limit(limit).all()
    ...
    try:
        ...
        for comm in comms:
            ...
            if comm.name and len(comm.name) < 5:
                continue
            items = session.query(Item).join(Item.collections).join(Collection.community).filter(Community.id==comm.id).all()
            if not items:
                continue
...
def department_comparison():
    ...
    try:
        ...
        for coll in comm.collections:
            ...
            years = [i.publication_date.year for i in items if i.publication_date]
            result[coll.name] = {"total":len(items),"open_access":oa,"restricted":len(items)-oa,"oa_rate":round(oa/len(items)*100,1) if items else 0,"unique_authors":len(set(a.name for i in items for a in i.authors)),"years":sorted(set(years))}
        return jsonify(result)
    finally:
        session.close()
...
def lecturer_profile():
    ...
    try:
        ...
        faculties = list(set(c.community.name for i in items for c in i.collections if c.community))
        ...
        return jsonify({"name":author.name,"total_papers":len(items),"open_access":oa,"oa_rate":round(oa/len(items)*100,1) if items else 0,"active_years":years,"faculties":faculties,"departments":depts,"top_collaborators":[{"name":n,"papers":c} for n,c in top_co],"papers":[{"id":i.id,"title":i.title,"doi":i.doi,"year":i.publication_date.year if i.publication_date else None,"is_oa":'openAccess' in (i.dc_rights or '')} for i in sorted(items,key=lambda x:x.publication_date or __import__('datetime').datetime.min,reverse=True)[:20]]})
    finally:
        session.close()
...
def language_research():
    ...
    try:
        ...
        STOP = {'the','and','for','with','this','that','from','have','been','were','their','which','these','about','other','into','than','more','such','some','what','when','where','there','also','using','used','study','show','paper','research','analysis','findings','results','between','effect','impact','based','data','method','approach','model','system','review','case','report'}
        matches = []
        keyword_counts = {}
        for item in items:
            try:
                ...
                matches.append({"id":item.id,"title":item.title,"year":item.publication_date.year if item.publication_date else None,"authors":[a.name for a in item.authors[:4]],"is_oa":'openAccess' in (item.dc_rights or ''),"score":score,"matched_terms":list(set(t1+t2))[:6]})
            except Exception as e:
        ...
        return jsonify({"total_language_papers":len(matches),"top_keywords":[{"keyword":k,"count":v} for k,v in top_keywords],"papers":matches[:50]})
    except Exception as e:
...
def generate_senate_report():
    ...
    try:
        ...
        if format_type == 'json':
            ...
            writer.writerow(['Institution', 'ROR', 'Total Papers', 'Total Authors',
                             'OA Rate %', 'TK Rate %', 'Patent Rate %',
                             'African Lang Rate %', 'Growth Rate %',
                             'Papers/Author', 'DocID Coverage %'])
            for inst in report['detailed_comparison']['institutions']:
                m = inst['metrics']
                writer.writerow([
                    inst['name'], inst['ror_id'],
                    m.get('total_papers', 0), m.get('total_authors', 0),
                    m.get('oa_rate', 0), m.get('tk_rate', 0),
                    m.get('patent_rate', 0), m.get('african_lang_rate', 0),
                    m.get('growth_rate', 0), m.get('papers_per_author', 0),
                    m.get('docid_coverage', 0),
                ])
            output.seek(0)
            return Response(output.getvalue(), mimetype='text/csv',
                            headers={'Content-Disposition': 'attachment; filename=senate_report.csv'})
            ...
            return Response('\n'.join(lines), mimetype='text/plain',
                            headers={'Content-Disposition': 'attachment; filename=senate_report.txt'})
        else:
...
def advanced_search():
    ...
    try:
        ...
        sort_by = request.args.get('sort', 'relevance')  # relevance, date, citations, title
...
def export_special_collections_csv():
    ...
    try:
        ...
        return Response(
            output.getvalue(),
            mimetype='text/csv',
            headers={'Content-Disposition': 'attachment; filename=uraas_special_collections.csv'}
        )
    except Exception as e:
...
def staff_directory():
    ...
    result = []
    insts = [registry.get(institution_filter)] if institution_filter else registry.list_all()
...
def docid_stats():
    ...
    try:
        ...
        return jsonify({"total_docid_papers":with_docid,"docid_coverage":round(with_docid/total*100,1) if total else 0})
    finally:
        session.close()
...
def export_csv():
    ...
    try:
        ...
        output = io.StringIO()
        writer = csv.writer(output)
        writer.writerow(['ID','DocID','Title','Authors','DOI','Year','Faculty','Open Access','Source'])
        for i in items:
            ...
            writer.writerow([i.id,i.docid or '',i.title or '',authors,i.doi or '',year,faculty,'Yes' if 'openAccess' in (i.dc_rights or '') else 'No',i.source_repository or ''])
        output.seek(0)
        return Response(output.getvalue(),mimetype='text/csv',headers={'Content-Disposition':'attachment; filename=uraas_papers.csv'})
    finally:
        session.close()
...
def export_bibtex():
    ...
    try:
        ...
        return Response('\n\n'.join(entries),mimetype='text/plain',headers={'Content-Disposition':'attachment; filename=uraas_papers.bib'})
    finally:
        session.close()
...
def start_crawler():
    ...
    with crawler_lock:
        ...
        try:
            ...
            project_root = os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
            script_path = os.path.join(project_root, 'scripts', 'crawl_multi_institution.py')
            ...
            thread = threading.Thread(target=crawler_monitor, args=(process,), daemon=True)
            thread.start()
            return jsonify({"status": "success", "message": f"Crawler started  target {target} papers"})
        except FileNotFoundError:
            return jsonify({"status": "error", "message": f"Crawler script not found at {script_path}"}), 500
        except Exception as e:
...
def start_docid_crawler():
    ...
    with docid_crawler_lock:
        if docid_crawler_process and docid_crawler_process.poll() is None:
            return jsonify({"status":"error","message":"DocID crawler already running"}),400
        data = request.get_json() or {}
        ...
        try:
            ...
            process = subprocess.Popen(
                [__import__('sys').executable, script_path, '--target', str(target), '--institutions', 'unilag'],
                stdout=subprocess.PIPE,stderr=subprocess.STDOUT,bufsize=1
            )
            docid_crawler_process = process
            thread = threading.Thread(target=docid_crawler_monitor,args=(process,),daemon=True)
            thread.start()
            return jsonify({"status":"success","message":f"DocID crawler started  target {target} papers"})
        except FileNotFoundError:
            return jsonify({"status":"error","message":"DocID crawler script not found"}),500
        except Exception as e:
...
def docid_crawler_status():
    with docid_crawler_lock:
        running = docid_crawler_process is not None and docid_crawler_process.poll() is None
    return jsonify({"status":"running" if running else "idle"})
