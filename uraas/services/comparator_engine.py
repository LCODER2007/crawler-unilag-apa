...
from uraas.database import SessionLocal, Item, Author, Community, Collection, File, item_authors
...
class InstitutionProfile:
    ...
    def calculate_metrics(self, session):
        ...
        tk_count = sum(1 for i in items if i.tk_label or i.content_type == 'indigenous_knowledge')
        ...
        self.metrics['patent_rate'] = round(patent_count / len(items) * 100, 1) if items else 0
        
        # African Languages
        african_lang_count = sum(1 for i in items if i.is_african_language)
        self.metrics['african_language_papers'] = african_lang_count
        self.metrics['african_lang_rate'] = round(african_lang_count / len(items) * 100, 1) if items else 0
        ...
        if years:
            ...
            self.metrics['growth_rate'] = round((recent - previous) / previous * 100, 1) if previous else 0
        else:
        ...
        # Efficiency ratios
        self.metrics['papers_per_author'] = round(len(items) / self.metrics['total_authors'], 2) if self.metrics['total_authors'] else 0
        self.metrics['patents_per_100_papers'] = round(patent_count / len(items) * 100, 1) if items else 0
...
class ComparatorEngine:
    ...
    def generate_senate_report(ror_ids: List[str], format: str = 'json') -> Dict:
        ...
        report = {
            ...
            'executive_summary': {
                'total_papers': sum(i['metrics']['total_papers'] for i in comparison['institutions']),
                'total_authors': sum(i['metrics']['total_authors'] for i in comparison['institutions']),
                'average_oa_rate': round(sum(i['metrics']['oa_rate'] for i in comparison['institutions']) / len(ror_ids), 1),
