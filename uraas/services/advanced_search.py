        ...
        try:
            ...
            if filters:
                if filters.get('year_from'):
                    q = q.filter(db_year(Item.publication_date) >= str(filters['year_from']))

                if filters.get('year_to'):
                    q = q.filter(db_year(Item.publication_date) <= str(filters['year_to']))
                
                if filters.get('oa_only'):
                    q = q.filter(Item.dc_rights.like('%openAccess%'))
                
                if filters.get('faculty'):
                    q = q.join(Item.collections).join(Collection.community).filter(
                        Community.name.ilike(f'%{filters["faculty"]}%')
                    )
            ...
            if sort_by == 'date':
                ...
                q = q.outerjoin(CitationMetrics, CitationMetrics.item_id == Item.id).order_by(
                    CitationMetrics.citation_count.desc().nullslast()
                )
            ...
            for item in results:
                formatted_results.append({
                    ...
                    'faculty': item.collections[0].community.name if item.collections and item.collections[0].community else None,
                    'department': item.collections[0].name if item.collections else None,
                    'is_oa': 'openAccess' in (item.dc_rights or ''),
                    'docid': item.docid,
                    'language': item.language_code,
                    'keywords': item.ai_keywords.split(',')[:5] if item.ai_keywords else []
                })
    ...
    def get_search_suggestions(partial_query: str, field: str = 'all') -> List[str]:
        ...
        try:
            ...
            if field in ('author', 'all'):
                authors = session.query(Author.name).filter(
                    Author.name.ilike(f'%{partial_query}%')
                ).limit(10).all()
                suggestions.extend([f"author:{a[0]}" for a in authors])
            
            if field in ('faculty', 'all'):
                faculties = session.query(Community.name).filter(
                    Community.name.ilike(f'%{partial_query}%')
                ).limit(5).all()
                suggestions.extend([f"faculty:{f[0]}" for f in faculties])
            
            if field in ('keyword', 'all'):
                # Extract keywords from papers
                items = session.query(Item.ai_keywords).filter(
                    Item.ai_keywords.ilike(f'%{partial_query}%')
                ).limit(20).all()
